...
import data.CSVMemoryInventoryRepository;
...
public class InventoryServiceTest {
    ...
    void setUp() { // Inject a fresh in-memory repository into the service before each test
        service = new InventoryService(new CSVMemoryInventoryRepository());
    }
    @Test
    void testAddProduct() { // Add a new product and check if the operation was successful
        Product product = new Product("P001", "Notebook", 10, new BigDecimal("2.99"));
    ...
    void testAddDuplicateProductFails() { // Add the same product twice; second attempt should fail
        Product product = new Product("P002", "Pen", 20, new BigDecimal("1.50"));
    ...
    void testRemoveProduct() {
        ...
        Product product = new Product("P004", "Eraser", 3, new BigDecimal("0.50"));
        service.addProduct(product);
    ...
    void testUpdateProduct() {
        ...
        Product original = new Product("P003", "Marker", 5, new BigDecimal("0.99"));
        service.addProduct(original);
        Product updated = new Product("P003", "Marker", 8, new BigDecimal("1.25"));
    ...
    void testFindProductById() {
        ...
        Product product = new Product("P005", "Glue", 6,new BigDecimal("1.10") );
        service.addProduct(product);
    ...
    void testGetAllProducts() {
        ...
        service.addProduct(new Product("P006", "Scissors", 4,new BigDecimal("2.75")));
        service.addProduct(new Product("P007", "Stapler", 2, new BigDecimal("4.99")));
        assertEquals(2, service.getAllProducts().size());
    }
    @Test
    void shouldRejectNegativeQuantity() {
        Product product = new Product("P008", "Negative Item", -5, new BigDecimal("10.0"));
        assertFalse(service.addProduct(product));
    }

    @Test
    void shouldRejectNegativePrice() {
        Product product = new Product("P009", "Negative Price", 5, new BigDecimal("-10.0"));
        assertFalse(service.addProduct(product));
    }
